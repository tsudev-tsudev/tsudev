...
export default function VerifyCertificate({ state, cert, serial }) {
  if (state !== 'found') {
  ...
  const meta = STATUS_META[cert.status] || STATUS_META.EXPIRED;
  const basis = BASIS_META[cert.basis] || {};
...
export async function getServerSideProps({ params, res }) {
  ...
  return { props: { state: result.state, cert: result.certificate || null, serial } };
}
